...
double compute_emat() {
   ...
   /* long int tlb_misses     = total_accesses - count_tlbhits; */
    long int total_accesses = count_writes + count_reads;
    long int tlb_misses = total_accesses - count_tlbhits;
    long int hitTime = MEMORY_ACCESS_TIME * count_tlbhits;
    long int MissTime = MEMORY_ACCESS_TIME * tlb_misses * 2;
    long int pageFaultTime = DISK_ACCESS_TIME * count_pagefaults;
   return 1.0 * (hitTime + MissTime + pageFaultTime) / total_accesses;
}
