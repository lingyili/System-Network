...
pfn_t pagefault_handler(vpn_t request_vpn, int write) {
  ...
  if (victim_pcb != NULL) {
    if (victim_pcb->pagetable[victim_vpn].valid == 1) {
      ...
      victim_pcb->pagetable[victim_vpn].used = 1;
  ...
  /* FIX ME */
  rlt[victim_pfn].vpn = request_vpn; //set victim_pfn's vpn to request_vpn
  rlt[victim_pfn].pcb = current;     //update current running on the CPU
