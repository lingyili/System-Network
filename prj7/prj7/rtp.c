...
static PACKET* packetize(char *buffer, int length, int *count){
	 ...
	 *count = size;
	 PACKET *packets = (PACKET*)malloc(sizeof(struct _PACKET) * size);
	 PACKET *temp;
	 int i = 0;
	 int posi = 0;
	 for (i = 0; i < length; i++) {
	 	posi = (i % MAX_PAYLOAD_LENGTH);
	 	temp = (packets + (i / MAX_PAYLOAD_LENGTH));
	 	temp->payload[posi] = buffer[i];
	 	if (i == (length - 1)) {
	 		temp->type = LAST_DATA;
	 		temp->payload_length = posi + 1;
...
int rtp_send_message(CONN_INFO *connection, MESSAGE*msg){
	 ...
	 */
	 int count = 0;
	 int i = 0;
	 PACKET *flag = (PACKET*)malloc(sizeof(PACKET));
	 ...
	 while (i < count){
	 	...
	 	recvfrom(connection->socket, flag, sizeof(PACKET), 0, connection->remote_addr, &connection->addrlen);
	 	if (flag->type == ACK) {
	 		i++;
	 	}
	 }
...
MESSAGE* rtp_receive_message(CONN_INFO *connection){
	 ...
	 */
	 ...
	 while (!data_sent) {
	 	PACKET p;
	 	recvfrom(connection->socket, &p, sizeof(PACKET), 0, NULL, NULL);
	 	if (checksum(p.payload, p.payload_length) == p.checksum) {
	 		response.type = ACK;
	 		if (p.type == LAST_DATA) {
	 			data_sent = 1;
	 		}
	 		int total_length = p.payload_length + msg->length;
	 		...
	 		for (i = 0; i < p.payload_length; i++) {
	 			buffer[i + msg->length] = p.payload[i];
	 		}
	 		msg->buffer = buffer;
	 		msg->length = msg->length + p.payload_length;
